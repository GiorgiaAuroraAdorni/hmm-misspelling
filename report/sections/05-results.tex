...
Introducing lemmatisation gave us a margin of +-2\% overall accuracy on the models, as, for example, run and 
running would both end up looking for P(run), and words that didn't have a corrispective term in the language 
model most probably had the originating lemma in it. But we also found the models would be losing precision in 
the lookup of different words that had the same originating lemma (non ricordo un esempio), and also the 
models would lose the nuance in probability between, for example, run and running, which have a different 
frequency of use.

The biggest issue we found was a large overhead of time consumption that the lemmatisation would take in the 
process of generating candidates, making the process last from an original 0.02 seconds (giusto?) to up to 5 
seconds, which we deemed unreasonable for the usage we had in mind for the project, thus abandoning the idea 
of lemmatisation.
